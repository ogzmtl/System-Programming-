...
int main(int argc, char *argv[]){
    ...
    {
        pthread_create(&thread_pool[i], NULL, consumeClient, &req);
    }
    while(1)
    {
        ...
        if(clientNumber <= max_clients ){
            ...
            if(oldPid != reqMainFifo.pid && reqMainFifo.inp_pid == getpid()){
                ...
                // enqueueRequest(reqMainFifo.pid, req.data, max_clients);
                write(STDOUT_FILENO, stringBuffer, sizeofString);
                while(1)
                {
...
void* consumeClient(void* arg)
{
    ...
    char client_fifo[CLIENT_FIFO_TEMP_LEN];
    snprintf(client_fifo, CLIENT_FIFO_TEMP_LEN,CLIENT_FIFO_TEMP, (long) req->pid);
    client_fd = open(client_fifo, O_RDONLY);

    while (read(client_fd, req, sizeof(struct request)) > 0) {
...
void sendResponse(int client_id, const char* request, int max_client, struct response* resp) {
    ...
    client_request.pid = client_id;
    strcpy(client_request.data, request);
